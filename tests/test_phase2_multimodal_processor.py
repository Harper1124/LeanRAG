...
class Phase2MultimodalProcessorTest(unittest.TestCase):
    ...
    def test_processing_does_not_create_or_modify_graph_edges(self):
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertIn("structured_content", reloaded[0])
            self.assertIn("semantic_content", reloaded[0])
            self.assertNotIn("content_understanding", reloaded[0])
            self.assertNotIn("semantic_understanding", reloaded[0])
